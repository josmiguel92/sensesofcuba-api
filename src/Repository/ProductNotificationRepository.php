<?php

namespace App\Repository;

use App\Entity\ProductNotification;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Doctrine\Persistence\ManagerRegistry;

/**
 * @method ProductNotification|null find($id, $lockMode = null, $lockVersion = null)
 * @method ProductNotification|null findOneBy(array $criteria, array $orderBy = null)
 * @method ProductNotification[]    findAll()
 * @method ProductNotification[]    findBy(array $criteria, array $orderBy = null, $limit = null, $offset = null)
 */
class ProductNotificationRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, ProductNotification::class);
    }

    // /**
    //  * @return ProductNotification[] Returns an array of ProductNotification objects
    //  */
    /*
    public function findByExampleField($value)
    {
        return $this->createQueryBuilder('p')
            ->andWhere('p.exampleField = :val')
            ->setParameter('val', $value)
            ->orderBy('p.id', 'ASC')
            ->setMaxResults(10)
            ->getQuery()
            ->getResult()
        ;
    }
    */

    /*
    public function findOneBySomeField($value): ?ProductNotification
    {
        return $this->createQueryBuilder('p')
            ->andWhere('p.exampleField = :val')
            ->setParameter('val', $value)
            ->getQuery()
            ->getOneOrNullResult()
        ;
    }
    */
}
